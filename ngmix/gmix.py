...
'''
class GMixSersic10Old(GMixSersicBase):
    """
    approximate sersic profiles using gaussians
    """
    p0spline = scipy.interpolate.CubicSpline(
...
class GMixSersic10(GMixSersicBase):
    ...
    spline = scipy.interpolate.CubicSpline(
        _sersic_data_10gauss['n'],
        _sersic_data_10gauss['fp'],
        extrapolate=True,
    )
    #fspline = scipy.interpolate.CubicSpline(
    #    _sersic_data_10gauss['n'],
    #    _sersic_data_10gauss['f']
    #)
    ...
    def _fill_pvals_fvals(self, n):
        ...
        fpvals = GMixSersic10.spline(n)
        #pvals = GMixSersic10.pspline(n)
        #fvals = GMixSersic10.fspline(n)

        fvals = self._fvals
        pvals = self._pvals
        ngauss=self._ngauss
        fvals[:] = fpvals[0:ngauss]
        pvals[:] = fpvals[ngauss:]
